...
GroceryStore::GroceryStore( const std::string & persistentInventoryDB )
{
 std::ifstream fin( persistentInventoryDB );                     // Creates the stream object, and then opens the file if it can
  ...
  while( fin >> std::quoted( upcCode ) >> quantity )
  {
    _inventoryDB.insert( { upcCode, quantity } );
  }
...
GroceryStore::GroceryItemsSold GroceryStore::ringUpCustomer( const ShoppingCart & shoppingCart, std::ostream & receipt )
{
  ...
  {
    std::string   upcCode = it->first;

    GroceryItem * item    = worldWideGroceryDatabase.find( upcCode );

    if(!item )
    {
      receipt << "\t" << std::quoted( upcCode ) << " (" << it->second.productName() << ")"
              << " not found, the item is free!" << std::endl;
    }
    else
    {
      receipt << "\t" << *item << std::endl;
      amountDue += item->price();
      auto inventoryIt = _inventoryDB.find( upcCode );

      if( inventoryIt != _inventoryDB.end() )
      {
        inventoryIt->second--;
        purchasedGroceries.insert( inventoryIt->first );
      }
    }
  }

  receipt << "-------------------------" << std::endl<<
  "Total  " << amountDue << std::endl << std::endl;
  /////////////////////// END-TO-DO (4) ////////////////////////////
...
void GroceryStore::reorderItems( GroceryItemsSold & todaysSales, std::ostream & reorderReport )
{
  ...
  reorderReport << std::endl << "Re-ordering grocery items the store is running low on.\n\n";

  int itemIndex = 1;
  for( auto saleIt = todaysSales.begin(); saleIt != todaysSales.end(); ++saleIt )
  {
    std::string upcCode       = *saleIt;

    auto inventoryIt = _inventoryDB.find( upcCode );


    if( inventoryIt == _inventoryDB.end() || inventoryIt->second < REORDER_THRESHOLD )
    {

      auto * groceryItem = worldWideGroceryDatabase.find( upcCode );

      if( !groceryItem)
      {
        reorderReport << " " << itemIndex
        << ":  {" << upcCode << "}" << std::endl;
      }
      else
      {
        reorderReport << " " << itemIndex
        << ":  {" << *groceryItem << "}" << std::endl;
      }
      ...
      else
      {
        ...
        int itemsToOrder = REORDER_THRESHOLD - inventoryIt->second;
        reorderReport << "        only " << inventoryIt->second << " remain in stock which is "
                      << itemsToOrder << " unit(s) below reorder threshold (15), re-ordering "
                      << LOT_COUNT << " more" << std::endl;
        inventoryIt->second += LOT_COUNT;
      }
