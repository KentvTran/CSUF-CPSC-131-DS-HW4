...
GroceryItem::GroceryItem( std::string productName, std::string brandName, std::string upcCode, double price )
  ...
  : _upcCode( std::move( upcCode ) ), _brandName( std::move( brandName ) ),
  _productName( std::move( productName ) ), _price( std::move( price ) )
{}
         /////////////////////// END-TO-DO (2) ////////////////////////////
...
GroceryItem::GroceryItem( GroceryItem && other ) noexcept
  ///////////////////////// TO-DO (4) //////////////////////////////
  : _upcCode( std::move( other._upcCode ) ), _brandName( std::move( other._brandName ) ),
   _productName( std::move( other._productName ) ), _price( std::move( other._price ) )
{}
         /////////////////////// END-TO-DO (4) ////////////////////////////
...
bool GroceryItem::operator==( const GroceryItem & rhs ) const noexcept
{
  ...
  ///////////////////////// TO-DO (20) //////////////////////////////
  return ( floating_point_is_equal(_price,rhs._price )) && ( _upcCode == rhs._upcCode ) && ( _productName == rhs._productName ) && _brandName == rhs._brandName;
  /////////////////////// END-TO-DO (20) ////////////////////////////
}
...
std::istream & operator>>( std::istream & stream, GroceryItem & groceryItem )
{
  ...
  if( stream >> std::ws >> std::quoted( workingItem._upcCode )
  && stream >> std::ws >> delimiter && delimiter == ','
  && stream >> std::ws >> std::quoted( workingItem._brandName )
  && stream >> std::ws >> delimiter && delimiter == ','
  && stream >> std::ws >> std::quoted( workingItem._productName )
  && stream >> std::ws >> delimiter && delimiter == ','
...
std::ostream & operator<<( std::ostream & stream, const GroceryItem & groceryItem )
{
  ...
  return stream << std::quoted( groceryItem._upcCode ) << comma
